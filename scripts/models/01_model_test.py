...
def main(args):
    ...
    # fp_train, fp_test = train_test_split(fp_df, test_size=0.15, random_state=42)
    # cb_train, cb_test = train_test_split(cb_df, test_size=0.15, random_state=42)

    # fp_x_train = np.array(fp_train["fingerprint"].tolist())
    # fp_y_train = np.array(fp_train[label].tolist())
    # fp_x_test = np.array(fp_test["fingerprint"].tolist())
    # fp_y_test = np.array(fp_test[label].tolist())
    # cb_x_train = np.array(cb_train["features"].tolist())
    # cb_y_train = np.array(cb_train[label].tolist())
    # cb_x_test = np.array(cb_test["features"].tolist())
    # cb_y_test = np.array(cb_test[label].tolist())

    fp_rfc = RandomForestClassifier(random_state=42, n_jobs=40)
    cb_rfc = RandomForestClassifier(random_state=42, n_jobs=40)
    # fp_knn = KNeighborsClassifier(n_jobs=40, )
    # cb_knn = KNeighborsClassifier(n_jobs=40, )
    # fp_svc = SVC(random_state=42)
    # cb_svc = SVC(random_state=42)
    # fp_mlpc = MLPClassifier(random_state=42, )
    # cb_mlpc = MLPClassifier(random_state=42, )
    # fp_gbc = GradientBoostingClassifier(random_state=42, )
    # cb_gbc = GradientBoostingClassifier(random_state=42, )

    fp_models = [fp_rfc]
    # fp_models = [fp_rfc, fp_knn, fp_mlpc, fp_gbc, fp_svc]
    cb_models = [cb_rfc]
    # cb_models = [cb_rfc, cb_knn, cb_mlpc, cb_gbc, cb_svc]
