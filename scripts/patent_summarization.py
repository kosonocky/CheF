...
def chatgpt_summarization(api_key, model = "gpt-3.5-turbo", system_prompt = "", user_prompt = "", abstract = "", description = "", desc_len = 1000, gpt_temperature=0):
    """
    Summarizes patent abstract and description for a given molecule using OpenAI GPT Model.
    ...
    --------
    summary : str
        Summary of the molecule. Returns empty string if any of the parameters are empty.
    """
    if (abstract == "") or (description == "") or (user_prompt == "") or (system_prompt == "") or (api_key == "") or (model == ""):
        return ""
    api_success = False
    tries = 0
    while api_success == False:
        try:
            ...
            if (desc_len != 0) and not isinstance(description, type(None)):
...
def summarization_wrapper(patent_info, api_key, model = "gpt-3.5-turbo", system_prompt = "", user_prompt = "", desc_len = 1000, gpt_temperature=0, gpt_max_patents_per_mol=10):       
    ...
    if patent_info == {}: # abort if nothing to summarize
        return patent_info
    summarizations = dict()
    patents_summarized = 0
    for key, value in patent_info.items():
        summ = chatgpt_summarization(api_key = api_key,
                                     model = model,
        ...
        patents_summarized += 1
        if gpt_max_patents_per_mol != -1: # never break if limit set to -1
            if patents_summarized >= gpt_max_patents_per_mol: # break if limit reached
                break
